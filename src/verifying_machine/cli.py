...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Verifying-Machine thinks for you about the meanings.",
        epilog="Example:  verifying-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | verifying-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | verifying-machine         #

    $ verifying-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="verifying-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"verifying-machine: {utterance}.")
        syslog.closelog()
